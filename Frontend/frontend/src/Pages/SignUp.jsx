...
function SignUp() {
  ...
  return (
    <div className='w-full h-[100vh] bg-cover flex justify-center items-center flex-col space-between' style={{backgroundImage:`url(${bg})`}}>
      <form className='w-[90%] h-[600px] mb-10 max-w-[500px] bg-[#00000081] backdrop-blur shadow-black flex flex-col justify-center items-center gap-20px px-[20px]' onSubmit={handleSignUp}>
      <h1 className='text-white text-2xl mb-[60px] font-semibold '>Register to <span className='text-blue-400 text-2xl'>Virtual Assistant</span></h1> 
      <input type="text" placeholder='Enter your Name' className='w-full h-[60px] outline-none border-2 border-white bg-transparent text-white placeholder-gray-300 px-[20px] py-[10px] rounded-full text-[18px] mb-5' required onChange={(e)=>setName(e.target.value)} value={name}/>
      ...
      <input type="email" placeholder='Email' className='w-full h-[60px] outline-none border-2 border-white bg-transparent text-white placeholder-gray-300 px-[20px] py-[10px] rounded-full text-[18px] mb-5' required onChange={(e)=>setEmail(e.target.value)} value={email}/>
        ...
        <button className='min-w-[150px] h-[60px] bg-white rounded-full text-black font-semibold mt-10 text-[19px]' disabled={loading}>{loading?"Loading...":"Sign Up"}</button>
        <p className='text-white text-[18px] cursor-pointer mt-4' onClick={() => navigate("/signin")}>Already have an account ? <span className='text-blue-400'>Sign In</span></p>
      </form>
    </div>
  )
}
